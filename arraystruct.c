...
void reduce(fraction_t* pf)
{
    ...
    int gcd_val = gcd ((*pf).num,(*pf).den);
    (*pf).num = ((*pf).num/gcd_val);
    (*pf).den = ((*pf).den/gcd_val);
}
...
{
	int num = (((*pf1).num*(*pf2).den)+((*pf2).num*(*pf1).den));
...
float calc_gpa(const grade_t *gradesp, int numGrades) {
    ...
    for(int i = 0; i<numGrades; i++){
        if(gradesp -> weight == 0.5){
            gpa_total = gpa_total + ((gradesp -> gp)/2);
            credit_total = credit_total + ((gradesp -> weight));
        }
        else {
            gpa_total = gpa_total + ((gradesp -> gp));
            credit_total = credit_total + ((gradesp -> weight));
        }
    	gradesp +=1;
    }
    return gpa_total/credit_total;
}
